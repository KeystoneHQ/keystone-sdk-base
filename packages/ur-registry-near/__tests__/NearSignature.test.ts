...
describe("near-signature", () => {
  it("test should generate near-signature", () => {
    ...
    const nearSignature = new NearSignature(signature, Buffer.from(idBuffer));

    const cborHex = nearSignature.toCBOR().toString("hex");
    const ur = nearSignature.toUREncoder(1000).nextPart();
    expect(ur).toBe(
      "ur:near-signature/oeadtpdagdndcawmgtfrkigrpmndutdnbtkgfssbjnaohdfzflvdrebeksfyamuroygtnettfneteebglugaskjnursamnuyaoskaajpcfktnyuewebgadkbdlnebyjtlskodmlnyaahstehckpdmyqzaxzmclmhbaammefwpazoehbafezmwpki"
    );
    ...
    expect(nearSignatureDecoded.getSignature().toString("hex")).toEqual(
      "47e7b510784406dfa14d9fd13c3834128b49c56ddfc28edb02c5047219779adeed12017e2f9f116e83762e86f805c7311ea88fb403ff21900e069142b1fb310e"
    );
  });
});
